...
class Analysis:
    ...
    def scatter(self, ind_var, dep_var, title):
        ...
        '''
        x = self.data.select_data(ind_var).reshape(self.data.get_num_samples(),) # select compatible data
        y = self.data.select_data(dep_var).reshape(self.data.get_num_samples(),) # select compatible data
        plt.scatter(x, y)
        plt.title(title)
        plt.xlabel(ind_var)
        plt.ylabel(dep_var)
    ...
    def pair_plot(self, data_vars, fig_sz=(12, 12), title=''):
        ...
        M = len(data_vars)
        selected_variables = self.data.select_data(data_vars) # select variables from dataset
        fig, axes = plt.subplots(nrows=M, ncols=M, sharex='col', sharey='row', figsize=fig_sz)
        fig.suptitle(title) # set title 
        for i in range(M):
            for j in range(M):
                axes[i, j].scatter(selected_variables[:,j], selected_variables[:,i]) # scatter plot
                if i == M-1: # if last row
                    axes[i, j].set_xlabel(data_vars[j]) # set x axis label
                if j == 0: # if first column
                    axes[i, j].set_ylabel(data_vars[i]) # set y axis label
        return fig, axes